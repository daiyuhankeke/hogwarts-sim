...
- **练习/上课/受指导场景必须更新 magic**：变形失败但有学习过程 → `subjects.transfiguration` +1~2；成功练习 → +2~3。例：
```json
"magic": { "subjects": { "transfiguration": 24 }, "spellsImproved": [{ "id": "wingardium_leviosa", "delta": 3 }] }
```
（前端也会根据剧情自动推断加成，但 DM 仍应主动写入 `stateUpdate.magic`）
