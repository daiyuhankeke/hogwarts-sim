...
export function validateStateUpdate(update) {
  ...
  if (update.player?.mood !== undefined) {
    const mood = update.player.mood;
    if (typeof mood !== 'number' || mood < 0 || mood > 100) {
      warnings.push('player.mood 应在 0-100');
    }
  }
...
export function sanitizeStateUpdate(update) {
  ...
  if (next.player?.mood !== undefined) {
    next.player.mood = Math.max(0, Math.min(100, Number(next.player.mood) || 0));
  }
