...
function getPromptsMtime() {
  const files = ['system-core.md', 'characters.md', 'magic-lore.md', 'world-lore.md', 'progression-lore.md'];
  return Math.max(
    ...files.map((f) => {
      try {
...
export function loadPrompts(includeWorldLore = true) {
  ...
  const combined = [core, world, magic, progression, characters].filter(Boolean).join('\n\n---\n\n');
