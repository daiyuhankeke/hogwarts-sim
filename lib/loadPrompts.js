...
function getPromptsMtime() {
  const files = ['system-core.md', 'characters.md', 'magic-lore.md', 'world-lore.md', 'progression-lore.md', 'timetable-lore.md', 'storyline-lore.md', 'careers-lore.md'];
  return Math.max(
    ...files.map((f) => {
      try {
...
export function loadPrompts(includeWorldLore = true) {
  ...
  const progression = fs.readFileSync(path.join(PROMPTS_DIR, 'progression-lore.md'), 'utf-8');
  const timetable = fs.readFileSync(path.join(PROMPTS_DIR, 'timetable-lore.md'), 'utf-8');
  const storyline = fs.readFileSync(path.join(PROMPTS_DIR, 'storyline-lore.md'), 'utf-8');
  let careers = '';
  try {
    careers = fs.readFileSync(path.join(PROMPTS_DIR, 'careers-lore.md'), 'utf-8');
  } catch {
    careers = '';
  }
  ...
  const combined = [core, world, magic, progression, timetable, storyline, careers, characters].filter(Boolean).join('\n\n---\n\n');
