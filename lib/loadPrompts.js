...
export function loadPrompts(includeWorldLore = true) {
  if (cachedPrompts && includeWorldLore) return cachedPrompts;
  ...
  const combined = [core, world, characters].filter(Boolean).join('\n\n---\n\n');

  if (includeWorldLore) cachedPrompts = combined;
  return combined;
}
